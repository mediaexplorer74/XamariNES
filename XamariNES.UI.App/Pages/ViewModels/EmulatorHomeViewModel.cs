...
namespace XamariNES.UI.App.Pages.ViewModels
{
    ...
    public class EmulatorHomeViewModel : ViewModelBase
    {
        ...
        {
            Task<Stream> GetImageStreamAsync(string[] filetypes); // for FilePicker
        ...
        public void MeasureFPS()
        {
            ...
            while (_powerOn)
            {
                ...
                //Console.WriteLine($"Measure FPS: {_framesRendered - previousFps}fps");
        ...
        /// <summary>
        /// 1 Paint Console's Top (with Power Button)
        ...
        void PaintConsole(SKPaintSurfaceEventArgs e)
        {
            ...
            var consoleRect = new SKRect(0, 0, e.Info.Rect.Right, (e.Info.Rect.Right / 6.04f)); // / 3.02f
            //RnD
            //var consoleRect = new SKRect(0, 0, 480, (480 / 4.02f));
            ...
            if (_powerOn)
            {
                ...
                var powerOnLightRect = new SKRect(135 * xRatio, 85 * yRatio, 150 * xRatio, 95 * yRatio);
            ...
            else
            {
                ...
                //RnD
                //var powerOnLightRect = new SKRect(135 * xRatio, 85 * yRatio, 150 * xRatio, 95 * yRatio);
                //e.Surface.Canvas.DrawRect(powerOnLightRect, new SKPaint() { Color = SKColors.LightYellow, Style = SKPaintStyle.StrokeAndFill });
            }
            ...
            _powerOnButtonRect = new SKRect(170 * xRatio, 75 * yRatio, 270 * xRatio, 115 * yRatio);

            //RnD
            //e.Surface.Canvas.DrawRect(_powerOnButtonRect, new SKPaint() { Color = SKColors.Blue, Style = SKPaintStyle.StrokeAndFill });

            // Load cartridge - TODO: Fix location on screen
            //_loadCartridgeButtonRect = new SKRect(170 * xRatio, 20 * yRatio / 6.04f, 370 * xRatio, 280 * yRatio / 6.04f);
            _loadCartridgeButtonRect = new SKRect(170 * xRatio, 20 * yRatio / 6.04f, 370 * xRatio, 280 * yRatio / 6.04f);

            //RnD
            //e.Surface.Canvas.DrawRect(_loadCartridgeButtonRect, new SKPaint() { Color = SKColors.Blue, Style = SKPaintStyle.StrokeAndFill });
        ...
        void PaintEmulator(SKPaintSurfaceEventArgs e)
        {
            ...
            if (!_powerOn)
            {
                ...
                e.Surface.Canvas.DrawBitmap(_renderer.Render(_frame),
                    new SKRect
                    (0,
                    _horizontalPadding,
                    e.Info.Rect.Right,
                    _screenHeight + _horizontalPadding)
                    );
                _framesRendered++;
                */
            }
            else
            {
                ...
                if (!_horizontalPaddingSet)
                {
                    ...
                    _screenHeight = (e.Info.Rect.Right / 1.066f); // 800
                    _horizontalPadding = (e.Info.Height - _screenHeight) / 2; // 0
                }
        ...
        /// <summary>
        /// 3 Paint Console's Bottom (with Controller)
        ...
        /// </summary>
        public async void SelectRomFile()
        {
            ...
            {
               //fileTypes = new string[] { ".nes" };
               fileTypes = new string[] { ".nes", ".zip" };
            }

            
            Stream filestream = await DependencyService.Get<IFileService>().GetImageStreamAsync(fileTypes);

            LoadNewGameCartridge(filestream);
        ...
        private void LoadNewGameCartridge(Stream filestream)
        {
            if (filestream != null)
            {
                
                byte[] romData = ReadStreamData(filestream);
        ...
        //Load game data from selected file (only .nes file extension is supported now)
        /*
        private void LoadNewGameCartridge1(FileResult file)
        {
            if (file != null)
            {
                if (file.FileName.EndsWith("nes", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    using (FileStream stream = File.Open(file.FileName, FileMode.Open))
                    {
                        romData = new byte[stream.Length];
                        //await
                        stream.ReadAsync(romData, 0, (int)stream.Length);
                    }
